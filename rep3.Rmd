...
# Figures 3-5: Instrumental Variables
...
I was able to recreate Figure 3.
...
# Using and subsetting state.fips
data(state.fips)
state.fips <- unique(state.fips[,c("fips","abb")])
...
# Forloop; took an hour to run, so set eval to false
# Saved and committed the file outputs from this forloop
for (i in move.rows) {
  cat("this row: ", i, "\n")
...
# These formula calls provide a way to establish a formula for a regression without retyping all the inputs over and over every time it is run. In this case, the left of the ~ is left empty to allow the formula to be used to test various dependent variables. 
...
# Then, when update is called in combination with the formula that has been created, the inputs are easily loaded into the formula. 
...
```{r, echo=FALSE, fig.cap = "Figure 3. Characteristics of white out-migrants and in-migrants compared to white nonmigrants for high-slave and low-slave counties, where migration took place between 1935 and 1939. In the left panel, each point is the estimated difference between nonmigrants and out-migrants from high-slave areas (dots) and between nonmigrants and out-migrants from low-slave areas (triangles), conditional on 1860 covariates of the individual’s 1935 county of residence. The right panel is the same for in-migration, conditional on 1860 covariates of the individual’s 1939 county of residence."}
...
I was able to recreate Figure 4 for the most part.
...
# Doing this PSET, I learned that dev.off marks the end of some graphic,
# table, etc. which proved to be very helpful when sorting through replication data.

# Note: data for tables 1 and 6 were not readily accessible in the
# dataverse files which is why they are not replicated. 
```
